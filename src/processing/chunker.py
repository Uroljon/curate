...
def is_heading(line: str) -> bool:
    ...
    german_keywords = [
        'Kapitel', 'Abschnitt', 'Teil', 'Artikel', 'Paragraph',
        'Handlungsfeld', 'Maßnahme', 'Projekt', 'Ziel', 'Indikator',
        'Anhang', 'Anlage', 'Übersicht', 'Zusammenfassung'
    ]
    ...
    for i, line in enumerate(lines[:5]):
        ...
        if is_heading(line):
            ...
            if 'Handlungsfeld' in line or re.match(r'^\d+\.\s+[A-ZÄÖÜ]', line):
                ...
                field_name = re.sub(r'^Handlungsfeld\s*:?\s*', '', field_name, flags=re.IGNORECASE)
    ...
    return {
        ...
        "structural_stats": {
            ...
            "chunks_with_indicators": sum(
                1 for chunk in chunks
                if any(re.search(pattern, chunk, re.IGNORECASE) 
