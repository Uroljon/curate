...
class BenchmarkRunner:
    ...
    def benchmark_chunking_methods(self, pdf_path: str) -> dict[str, Any]:
        ...
        # Extract text
        start = time.time()
        text, metadata = extract_text_with_ocr_fallback(pdf_path)
        ...
        for size in chunk_sizes:
            ...
            results["chunking_methods"][f"semantic_{size}"] = {
                ...
                "chunks_with_headings": quality["heading_stats"][
            ...
            print(
                f"   Chunks: {len(chunks)}, Avg size: {quality['size_stats']['avg']:.0f}"
            )
            print(
                f"   With headings: {quality['heading_stats']['chunks_with_headings']}/{len(chunks)}"
            )
        ...
        # Old method
        start = time.time()
        old_llm_chunks = prepare_llm_chunks(
        ...
        results["llm_chunking_comparison"] = {
            "old_method": {
                ...
                "chunks_with_headings": old_quality["heading_stats"][
                    "chunks_with_headings"
                ],
            ...
            "new_method": {
                ...
                "chunks_with_headings": new_quality["heading_stats"][
                    "chunks_with_headings"
                ],
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                results["endpoints"]["fast"] = {
                    ...
                    "extraction_time": metadata.get(
                        "extraction_time_seconds", fast_time
                    ),
                    "chunks_processed": metadata.get("chunks_processed", 0),
                    "action_fields": len(structures),
                    "total_projects": metadata.get("total_projects", 0),
                    "projects_with_indicators": metadata.get(
                        "projects_with_indicators", 0
                    ),
                    "indicator_rate": (
                        metadata.get("projects_with_indicators", 0)
                        / metadata.get("total_projects", 1)
                        if metadata.get("total_projects", 0) > 0
                        else 0
                    ),
                }
                ...
                print(
                    f"   📋 Results: {len(structures)} action fields, {metadata.get('total_projects', 0)} projects"
                )
                print(
                    f"   📊 Indicator rate: {results['endpoints']['fast']['indicator_rate']:.1%}"
                )
            else:
                results["endpoints"]["fast"] = {
                    "success": False,
            ...
            results["endpoints"]["fast"] = {"success": False, "error": str(e)}
            print(f"   ❌ Error: {e}")
    ...
    def benchmark_multiple_pdfs(self, pdf_paths: list[str]) -> None:
        ...
        for pdf_path in pdf_paths:
            ...
            if upload_success:
                ...
                print("   ❌ Upload failed, skipping benchmarks")
    ...
    def upload_pdf(self, pdf_path: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(
                    f"   ✅ Uploaded successfully: {data.get('chunks', 0)} chunks created"
                )
                return True
            else:
        ...
        for benchmark in self.results["benchmarks"]:
            ...
            if "chunking" in benchmark:
                ...
                for method, stats in chunking.get("chunking_methods", {}).items():
                    ratio = (
                        stats["chunks_with_headings"] / stats["chunk_count"]
                ...
                if best_size:
                    print(
                        f"   Best chunk size: {best_size} ({best_heading_ratio:.1%} with headings)"
                    )
                ...
                if "llm_chunking_comparison" in chunking:
                    ...
                    print(
                        f"   LLM chunking: Old {old['chunk_count']} → New {new['chunk_count']} chunks"
                    )
                    print(
                        f"   Heading preservation: {old['chunks_with_headings']} → {new['chunks_with_headings']}"
                    )
            ...
            if "extraction" in benchmark:
                ...
                for endpoint, stats in extraction.get("endpoints", {}).items():
                    if stats.get("success"):
                        ...
                        print(
                            f"      Indicator rate: {stats.get('indicator_rate', 0):.1%}"
                        )
                    else:
...
def main():
    ...
    parser.add_argument(
        "--all", action="store_true", help="Benchmark all PDFs in uploads folder"
    )
    ...
    if performance_analysis.get("performance_stats"):
        ...
        print(
            f"   Min/Max: {stats['min_duration']:.2f}s - {stats['max_duration']:.2f}s"
        )
