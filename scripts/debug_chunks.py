...
def show_chunk_boundaries(source_id: str):
    ...
    for i, chunk in enumerate(raw_texts[:5]):
        ...
        if any(marker in chunk for marker in ["Handlungsfeld:", "Maßnahmen:", "Projekte:", "Indikatoren:"]):
    ...
    for i, chunk in enumerate(optimized[:3]):
        ...
        if marker_count > 1:
            print(f"  ⚠️  Contains {marker_count} section markers - mixing different sections!")
