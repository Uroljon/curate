...
def test_all_pdfs():
    ...
    for pdf_path in pdf_files:
        ...
        try:
            ...
            if success:
                ...
                status = "FAILED"
                results["summary"]["issues"].append(f"{pdf_path.name} - Integration test failed")
            ...
            results["summary"]["failed"] += 1
            results["summary"]["issues"].append(f"{pdf_path.name} - Exception: {str(e)}")
            results["pdfs_tested"].append({
                "pdf": pdf_path.name,
                "status": "ERROR",
                "error": str(e),
    ...
    # Save results
    results_file = Path("test_results") / f"pdf_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def run_extraction_tests():
    ...
    try:
        ...
        if response.status_code != 200:
            print("❌ FastAPI server not responding. Please start it with: uvicorn main:app --reload")
            return False
    except Exception as e:
...
def analyze_pipeline_logs():
    ...
    if extraction_analysis["total_events"] > 0:
        ...
        print(f"   Date range: {extraction_analysis['date_range']['start']} to {extraction_analysis['date_range']['end']}")
...
def main():
    ...
    parser.add_argument("--extraction", action="store_true", help="Run extraction tests via API")
