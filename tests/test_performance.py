    ...
    try:
        ...
        if response.status_code == 200:
            ...
            projects_with_measures = sum(
                1 for af in structures for p in af["projects"] if p.get("measures")
            )
            projects_with_indicators = sum(
                1 for af in structures for p in af["projects"] if p.get("indicators")
            )
...
def main():
    ...
    if results["multi_stage"]["success"] and results["fast"]["success"]:
        ...
        if multi["total_projects"] > 0:
            multi_indicator_rate = (
                multi["projects_with_indicators"] / multi["total_projects"]
            )
            fast_indicator_rate = (
                fast["projects_with_indicators"] / fast["total_projects"]
            )
