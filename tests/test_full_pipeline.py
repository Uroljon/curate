...
30% weniger PKW-Verkehr bis 2030."""
    ...
    try:
        ...
        cross_similarity = dot(mixed_embeddings[0], mixed_embeddings[1]) / (
            norm(mixed_embeddings[0]) * norm(mixed_embeddings[1])
        )
    ...
    try:
        ...
        print(
            f"   ✅ Chunks with headings: {quality['heading_stats']['chunks_with_headings']}/{quality['total_chunks']}"
        )
...
def test_embedding_storage():
    ...
    try:
        ...
        assert any(
            "Testbereich" in text for text in retrieved_texts
        ), "Should retrieve test content"
