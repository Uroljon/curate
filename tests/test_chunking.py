...
"""
    ...
    print(
        f"Reduction: {len(old_chunks) - len(new_chunks)} fewer chunks ({(1 - len(new_chunks)/len(old_chunks))*100:.1f}% reduction)"
    )
    ...
    assert all(
        len(c) <= 5000 for c in new_chunks
    ), "All chunks should be under 5000 chars"
...
if __name__ == "__main__":
    ...
    print("\n✅ All tests passed!")
    print(
        f"Summary: Reduced chunks from {old_count} to {new_count} ({(1-new_count/old_count)*100:.0f}% improvement)"
    )
