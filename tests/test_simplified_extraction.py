...
def test_simplified_extraction():
    ...
    try:
        ...
        response = requests.get(
            f"{base_url}/extract_structure_fast", params={"source_id": "regensburg.pdf"}
        )
    ...
    extraction_rate = (
        (total_indicators / expected_indicators) * 100 if expected_indicators > 0 else 0
    )
    ...
    print("  Target: >70%")
    print(
        f"  Status: {'✅ PASSED' if extraction_rate > 70 else '❌ NEEDS IMPROVEMENT'}"
    )
