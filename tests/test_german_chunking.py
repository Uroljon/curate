...
def test_german_heading_detection():
    ...
    test_cases = [
        ...
        ("Ein sehr langer Satz der definitiv kein Heading ist weil er zu lang ist und auch zu viele Wörter enthält.", False, "Too long"),
...
Die Stadt setzt sich ehrgeizige Ziele."""
    ...
    print(f"Test 1 - Heading with colon on separate line:")
...
Der Modal Split soll 30% PKW und 70% Umweltverbund betragen."""
    ...
    for indicator in indicators:
        ...
        if len(found_in_chunks) == 1:
            print(f"✅ '{indicator}' found in exactly one chunk (chunk {found_in_chunks[0]})")
        else:
            print(f"❌ '{indicator}' found in {len(found_in_chunks)} chunks: {found_in_chunks}")
