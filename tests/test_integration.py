...
def test_real_pdf_flow(pdf_path: str):
    ...
    try:
        ...
        print(f"   ✅ Extracted {len(extracted_text):,} characters in {extraction_time:.2f}s")
        ...
        print(f"   📋 Heading detection: {total_headings} headings in {chunks_with_headings}/{len(chunks)} chunks")
        ...
        # New semantic method
        new_chunks = prepare_semantic_llm_chunks(chunks, max_chars=20000, min_chars=15000)
        ...
        print(f"\n   📈 Quality comparison:")
        print(f"      Chunks with headings: {old_quality['heading_stats']['chunks_with_headings']} → {new_quality['heading_stats']['chunks_with_headings']}")
        print(f"      Mixed sections: {old_quality.get('mixed_sections', 0)} → {new_quality.get('mixed_sections', 0)}")
        ...
        if chunks_with_headings < len(chunks) * 0.3:
            issues.append(f"Only {chunks_with_headings}/{len(chunks)} chunks have headings")
        ...
        print(f"   Total processing time: {extraction_time + chunking_time + embedding_time:.2f}s")
...
def test_edge_cases():
    ...
    edge_cases = [
        ...
        ("Mixed languages", "Klimaschutz means climate protection und Mobilität means mobility"),
        ("Huge heading", "A" * 150),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Integration test for PDF extraction pipeline")
