...
from .config import AnalyzerConfig, DEFAULT_CONFIG, calculate_composite_score, get_quality_grade
from .models import (
    AnalysisResult, AnalysisMetadata, ComparisonResult, QualityScore,
    GraphStats, IntegrityStats, ConnectivityStats, ConfidenceStats, 
    SourceStats, ContentStats, DriftStats
)
from .metrics import (
...
class JSONAnalyzer:
    ...
    def __init__(self, config: Optional[AnalyzerConfig] = None):
        ...
        self.connectivity_metrics = ConnectivityMetrics(self.config.connectivity_thresholds)
    ...
    def compare_files(self, before_path: Union[str, Path], after_path: Union[str, Path]) -> ComparisonResult:
        """
        ...
        improvements, regressions = self._calculate_improvements_regressions(before_result, after_result)
        ...
        return ComparisonResult(
            ...
            summary=self._generate_comparison_summary(before_result, after_result, drift_stats)
        )
    ...
    def _calculate_quality_score(self, result: AnalysisResult) -> QualityScore:
        ...
        # Apply adjustments
        final_score = max(0.0, min(100.0, composite_score - sum(penalties.values()) + sum(bonuses.values())))
    ...
    def _score_integrity(self, stats: IntegrityStats) -> float:
        ...
        # Penalize duplicates
        avg_duplicate_rate = sum(stats.duplicate_rate.values()) / max(1, len(stats.duplicate_rate))
        ...
        if stats.field_completeness:
            ...
            if min_completeness < self.config.integrity_thresholds.min_field_completeness:
                score -= min(20, (self.config.integrity_thresholds.min_field_completeness - min_completeness) * 40)
    ...
    def _score_connectivity(self, stats: ConnectivityStats) -> float:
        ...
        if stats.action_field_coverage < self.config.connectivity_thresholds.min_af_coverage:
            score -= min(30, (self.config.connectivity_thresholds.min_af_coverage - stats.action_field_coverage) * 60)
        
        # Penalize low project coverage
        if stats.project_coverage < self.config.connectivity_thresholds.min_project_coverage:
            score -= min(25, (self.config.connectivity_thresholds.min_project_coverage - stats.project_coverage) * 50)
        ...
        if measures_mean < self.config.connectivity_thresholds.min_measures_per_project:
            score -= min(20, (self.config.connectivity_thresholds.min_measures_per_project - measures_mean) * 20)
    ...
    def _score_confidence(self, stats: ConfidenceStats) -> float:
        ...
        if stats.low_confidence_edges:
            total_low_conf = sum(edge_data["count"] for edge_data in stats.low_confidence_edges.values())
            total_edges = sum(edge_data["total"] for edge_data in stats.low_confidence_edges.values())
            
            if total_edges > 0:
                low_conf_ratio = total_low_conf / total_edges
                if low_conf_ratio > self.config.confidence_thresholds.max_low_confidence_ratio:
                    score -= min(40, (low_conf_ratio - self.config.confidence_thresholds.max_low_confidence_ratio) * 80)
        ...
        if stats.calibration_metrics:
            ...
            if mean_conf < self.config.confidence_thresholds.min_mean_confidence:
                score -= min(20, (self.config.confidence_thresholds.min_mean_confidence - mean_conf) * 40)
    ...
    def _score_sources(self, stats: SourceStats) -> float:
        ...
        if overall_coverage < self.config.source_thresholds.min_source_coverage:
            score -= min(40, (self.config.source_thresholds.min_source_coverage - overall_coverage) * 50)
        
        # Penalize low quote match rate
        if stats.quote_match_rate < self.config.source_thresholds.min_quote_match_rate:
            score -= min(30, (self.config.source_thresholds.min_quote_match_rate - stats.quote_match_rate) * 60)
    ...
    def _score_content(self, stats: ContentStats) -> float:
        ...
        if stats.repetition_rate > self.config.content_thresholds.max_repetition_rate:
            score -= min(30, (stats.repetition_rate - self.config.content_thresholds.max_repetition_rate) * 60)
        ...
        if inconsistency > self.config.content_thresholds.max_language_inconsistency:
            score -= min(25, (inconsistency - self.config.content_thresholds.max_language_inconsistency) * 50)
        
        # Penalize normalization issues and outliers
        score -= min(25, len(stats.normalization_issues) * 2)
        total_outliers = sum(len(outlier_list) for outlier_list in stats.outliers.values())
    ...
    def _calculate_bonuses(self, result: AnalysisResult) -> Dict[str, float]:
        ...
        if result.confidence_stats.calibration_metrics:
            mean_conf = result.confidence_stats.calibration_metrics.get("overall_mean_confidence", 0.0)
            if mean_conf > 0.9:
                bonuses["high_confidence"] = 2
        
        return bonuses

    def _calculate_improvements_regressions(self, before: AnalysisResult, after: AnalysisResult) -> tuple[Dict[str, float], Dict[str, float]]:
        """Calculate improvements and regressions between two results."""
        improvements = {}
        regressions = {}
        
        # Compare quality scores
        score_diff = after.quality_score.overall_score - before.quality_score.overall_score
        if score_diff > 0:
        ...
        # Compare coverage
        coverage_diff = after.connectivity_stats.action_field_coverage - before.connectivity_stats.action_field_coverage
        if coverage_diff > 0:
    ...
    def _generate_comparison_summary(self, before: AnalysisResult, after: AnalysisResult, drift_stats: DriftStats) -> str:
        """Generate human-readable comparison summary."""
        score_change = after.quality_score.overall_score - before.quality_score.overall_score
        ...
        stability_grade = self.drift_metrics.get_drift_summary(drift_stats)["stability_grade"]
...
def analyze_file(file_path: Union[str, Path], config: Optional[AnalyzerConfig] = None) -> AnalysisResult:
    """Convenience function to analyze a single file."""
    analyzer = JSONAnalyzer(config)
    return analyzer.analyze_file(file_path)

def compare_files(before_path: Union[str, Path], after_path: Union[str, Path], config: Optional[AnalyzerConfig] = None) -> ComparisonResult:
