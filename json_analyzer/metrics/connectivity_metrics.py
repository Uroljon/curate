...
class ConnectivityMetrics:
    ...
    def _calculate_action_field_coverage(self, graph: nx.Graph, data: Dict[str, Any]) -> float:
        """Calculate percentage of action fields that have incoming connections."""
        af_nodes = [node for node, data_attr in graph.nodes(data=True) 
    ...
    def _calculate_project_coverage(self, graph: nx.Graph, data: Dict[str, Any]) -> float:
        """Calculate percentage of projects connected to both action fields and measures."""
        project_nodes = [node for node, data_attr in graph.nodes(data=True) 
    ...
    def _calculate_measures_per_project(self, graph: nx.Graph, data: Dict[str, Any]) -> Dict[str, float]:
        ...
        for proj_node in project_nodes:
            neighbors = list(graph.neighbors(proj_node))
            measure_neighbors = [n for n in neighbors 
                               if graph.nodes[n].get('type') == 'measure']
    ...
    def _calculate_centrality_measures(self, graph: nx.Graph) -> Dict[str, Dict[str, float]]:
        ...
        try:
            ...
            if nx.is_connected(graph) and graph.number_of_nodes() <= self.thresholds.min_centrality_diversity * 1000:
                # Closeness centrality
                closeness_centrality = nx.closeness_centrality(graph)
                centrality_stats["closeness"] = self._centrality_statistics(closeness_centrality)
                
                # Betweenness centrality (expensive, limit to smaller graphs)
                if graph.number_of_nodes() <= 1000:
                    betweenness_centrality = nx.betweenness_centrality(graph)
                    centrality_stats["betweenness"] = self._centrality_statistics(betweenness_centrality)
            ...
            if graph.number_of_nodes() > 0:
                ...
                degree_centrality = {node: deg/max_degree for node, deg in degrees.items()}
                centrality_stats["degree"] = self._centrality_statistics(degree_centrality)
        
        return centrality_stats

    def _centrality_statistics(self, centrality_dict: Dict[str, float]) -> Dict[str, float]:
    ...
    def _calculate_detailed_coverage(self, graph: nx.Graph, data: Dict[str, Any]) -> Dict[str, Any]:
        """Calculate detailed coverage metrics."""
        node_types = ["action_field", "project", "measure", "indicator"]
        coverage_stats = {}
        
        for node_type in node_types:
            nodes = [node for node, data_attr in graph.nodes(data=True) 
            ...
            coverage_stats[node_type] = {
                ...
                "avg_connections": mean([graph.degree[node] for node in nodes]) if nodes else 0.0
            }
    ...
    def get_connectivity_patterns(self, graph: nx.Graph) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_common_patterns": sorted(patterns.items(), key=lambda x: x[1], reverse=True)[:10]
        }

    def get_hub_analysis(self, graph: nx.Graph, top_n: int = 10) -> Dict[str, List[Dict[str, Any]]]:
