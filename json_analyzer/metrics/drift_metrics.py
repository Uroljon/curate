...
class DriftMetrics:
    ...
    def calculate(self, before_data: Dict[str, Any], after_data: Dict[str, Any]) -> DriftStats:
        """
        ...
        structural_similarity = self._calculate_structural_similarity(before_data, after_data)
        ...
        stability_score = self._calculate_stability_score(
            node_churn, edge_churn, coverage_delta, confidence_drift, structural_similarity
        )
    ...
    def _calculate_node_churn(self, before_data: Dict[str, Any], after_data: Dict[str, Any]) -> Dict[str, int]:
        ...
        for entity_type in entity_types:
            ...
            modified = self._find_modified_nodes(
                before_data.get(entity_type, []), 
                after_data.get(entity_type, [])
            )
            ...
            churn[entity_type] = {
                ...
                "churn_rate": (len(added) + len(removed) + len(modified)) / max(1, len(before_nodes))
            }
    ...
    def _extract_nodes(self, entities: List[Dict[str, Any]]) -> Set[str]:
        ...
        for entity in entities:
            ...
            if entity_id:
                ...
                name = content.get("title") or content.get("name") or content.get("action_field", "")
                if name:
                    nodes.add(name)
        return nodes

    def _find_modified_nodes(self, before_entities: List[Dict[str, Any]], after_entities: List[Dict[str, Any]]) -> Set[str]:
        ...
        for entity_id in before_lookup:
            if entity_id in after_lookup:
                if self._entities_differ(before_lookup[entity_id], after_lookup[entity_id]):
                    modified.add(entity_id)
        
        return modified

    def _entities_differ(self, entity1: Dict[str, Any], entity2: Dict[str, Any]) -> bool:
    ...
    def _calculate_edge_churn(self, before_data: Dict[str, Any], after_data: Dict[str, Any]) -> Dict[str, int]:
    ...
    def _calculate_coverage_delta(self, before_data: Dict[str, Any], after_data: Dict[str, Any]) -> Dict[str, float]:
    ...
    def _calculate_basic_coverage(self, data: Dict[str, Any]) -> Dict[str, float]:
        ...
        for entity_type in entity_types:
            ...
            coverage[f"{entity_type}_with_connections"] = connected_entities / len(entities)
        
        return coverage

    def _calculate_confidence_drift(self, before_data: Dict[str, Any], after_data: Dict[str, Any]) -> Dict[str, float]:
    ...
    def _extract_confidence_scores(self, data: Dict[str, Any]) -> Dict[str, List[float]]:
    ...
    def _calculate_structural_similarity(self, before_data: Dict[str, Any], after_data: Dict[str, Any]) -> float:
    ...
    def _get_structural_signature(self, data: Dict[str, Any]) -> Set[str]:
        ...
        for entity_type in entity_types:
            ...
            signature.add(f"nodes_{entity_type}_{count//10*10}")  # Rounded to nearest 10
    ...
    def _calculate_overall_churn_rate(self, node_churn: Dict[str, int], edge_churn: Dict[str, int]) -> float:
        """Calculate overall churn rate."""
        total_node_churn = 0
        total_nodes = 0
    ...
    def _calculate_stability_score(
        ...
        avg_node_churn = mean([stats["churn_rate"] for stats in node_churn.values()]) if node_churn else 0.0
        avg_edge_churn = mean([stats["churn_rate"] for stats in edge_churn.values()]) if edge_churn else 0.0
        ...
        max_coverage_change = max([abs(delta) for delta in coverage_delta.values()]) if coverage_delta else 0.0
        score -= min(15, max_coverage_change * 30)  # Max 15 point penalty
        
        # Penalize large confidence drift
        max_confidence_drift = 0.0
        if confidence_drift:
            drift_values = [abs(stats["mean_delta"]) for stats in confidence_drift.values()]
    ...
    def get_drift_summary(self, stats: DriftStats) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_volatile_entity_type": self._find_most_volatile_type(stats.node_churn),
    ...
    def _generate_stability_recommendations(self, stats: DriftStats) -> List[str]:
        ...
        if stats.stability_score < 70:
            recommendations.append("High instability detected - consider stabilizing extraction process")
        ...
        if stats.structural_similarity < 0.8:
            recommendations.append("Low structural similarity - verify extraction approach consistency")
        
        # Check for high confidence drift
        if stats.confidence_drift:
            max_drift = max(abs(drift["mean_delta"]) for drift in stats.confidence_drift.values())
            if max_drift > 0.2:
                recommendations.append("Significant confidence drift detected - review confidence scoring")
