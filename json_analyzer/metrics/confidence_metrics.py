...
class ConfidenceMetrics:
    ...
    def _analyze_confidence_by_type(self, data: Dict[str, Any]) -> Dict[str, Dict[str, float]]:
        ...
        for conn_type, confidences in confidence_by_type.items():
            if confidences:
                stats_by_type[conn_type] = {
                    ...
                    "low_confidence_ratio": sum(1 for c in confidences if c < self.thresholds.low_confidence_threshold) / len(confidences)
                }
            else:
                stats_by_type[conn_type] = {
                    "mean": 0.0, "median": 0.0, "min": 0.0, "max": 0.0, 
                    "std": 0.0, "count": 0, "low_confidence_ratio": 0.0
                }
    ...
    def _detect_ambiguous_nodes(self, data: Dict[str, Any], graph: nx.Graph) -> List[Dict[str, Any]]:
        ...
        for entity_type in entity_types:
            for entity in data.get(entity_type, []):
                ...
                low_confidence_count = sum(1 for c in confidences if c < self.thresholds.low_confidence_threshold)
    ...
    def _analyze_confidence_distributions(self, data: Dict[str, Any]) -> Dict[str, List[float]]:
    ...
    def _calculate_calibration_metrics(self, data: Dict[str, Any]) -> Dict[str, float]:
        ...
        return {
            ...
            "high_confidence_ratio": sum(1 for c in all_confidences if c > 0.9) / len(all_confidences),
            "medium_confidence_ratio": sum(1 for c in all_confidences if 0.7 <= c <= 0.9) / len(all_confidences),
            "low_confidence_ratio": sum(1 for c in all_confidences if c < 0.7) / len(all_confidences)
        }
    ...
    def get_confidence_histogram(self, data: Dict[str, Any], bins: int = 10) -> Dict[str, Dict[str, int]]:
    ...
    def get_confidence_outliers(self, data: Dict[str, Any], threshold: float = 2.0) -> List[Dict[str, Any]]:
