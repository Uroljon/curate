...
class SourceMetrics:
    ...
    def _find_pages_file(self, json_file_path: str) -> Optional[str]:
    ...
    def _calculate_source_coverage(self, data: Dict[str, Any]) -> Dict[str, float]:
        ...
        coverage["overall"] = total_with_sources / total_entities if total_entities > 0 else 0.0
        
        return coverage

    def _validate_quotes(self, data: Dict[str, Any], page_text: Dict[int, str]) -> Tuple[float, List[Dict[str, str]]]:
    ...
    def _validate_page_numbers(self, data: Dict[str, Any], page_text: Dict[int, str]) -> Dict[str, Any]:
        ...
        page_stats["unique_pages_referenced"] = list(page_stats["unique_pages_referenced"])
    ...
    def _analyze_chunk_linkage(self, data: Dict[str, Any]) -> Dict[str, float]:
        ...
        if chunk_stats["total_sources"] > 0:
            chunk_stats["chunk_coverage"] = chunk_stats["sources_with_chunks"] / chunk_stats["total_sources"]
    ...
    def get_source_quality_report(self, data: Dict[str, Any], file_path: str = "") -> Dict[str, Any]:
        ...
        quality_factors = {
            ...
            "page_validity": stats.page_validity.get("valid_page_refs", 0) / max(1, stats.page_validity.get("total_page_refs", 1)),
            "chunk_linkage": stats.chunk_linkage.get("chunk_coverage", 0.0)
        }
        
        # Weighted quality score
        weights = {"coverage": 0.3, "quote_match": 0.4, "page_validity": 0.2, "chunk_linkage": 0.1}
    ...
    def _generate_recommendations(self, stats: SourceStats) -> List[str]:
        ...
        if stats.source_coverage.get("overall", 0) < self.thresholds.min_source_coverage:
            recommendations.append(f"Improve source coverage (currently {stats.source_coverage.get('overall', 0):.1%}, target: {self.thresholds.min_source_coverage:.1%})")
        
        if stats.quote_match_rate < self.thresholds.min_quote_match_rate:
            recommendations.append(f"Improve quote accuracy (currently {stats.quote_match_rate:.1%}, target: {self.thresholds.min_quote_match_rate:.1%})")
        
        if len(stats.invalid_quotes) > 0:
            recommendations.append(f"Fix {len(stats.invalid_quotes)} invalid quote references")
        
        if len(stats.missing_sources) > 0:
            recommendations.append(f"Add sources for {len(stats.missing_sources)} entities without attribution")
        
        page_validity_rate = stats.page_validity.get("valid_page_refs", 0) / max(1, stats.page_validity.get("total_page_refs", 1))
        if page_validity_rate < 0.95:
            recommendations.append(f"Fix page number references ({100 - page_validity_rate*100:.1f}% are invalid)")
