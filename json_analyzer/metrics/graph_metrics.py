...
class GraphMetrics:
    ...
    def _calculate_degree_by_type(self, graph: nx.Graph) -> Dict[str, Dict[str, float]]:
        ...
        for node_type, degrees in degree_by_type.items():
            if degrees:
                ...
                stats_by_type[node_type] = {
                    'mean': 0.0, 'median': 0.0, 'max': 0, 'min': 0, 'std': 0.0
                }
    ...
    def build_graph_from_data(self, data: Dict[str, Any]) -> nx.Graph:
        ...
        if "action_fields" in data and isinstance(data["action_fields"], list):
            if len(data["action_fields"]) > 0 and "projects" in data["action_fields"][0]:
    ...
    def _build_from_extraction_result(self, data: Dict[str, Any], graph: nx.Graph) -> nx.Graph:
    ...
    def _build_from_enriched_review(self, data: Dict[str, Any], graph: nx.Graph) -> nx.Graph:
