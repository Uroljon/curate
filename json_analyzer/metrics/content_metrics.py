...
class ContentMetrics:
    ...
    def _analyze_repetition(self, data: Dict[str, Any]) -> tuple[float, List[Dict[str, str]]]:
        ...
        for dup_text in all_duplicates:
            ...
            if len(entities) > 1:
                duplicate_text.append({
                    "text": dup_text[:100] + "..." if len(dup_text) > 100 else dup_text,
        ...
        repetition_rate = total_duplicated_texts / len(set(all_texts)) if all_texts else 0.0
    ...
    def _normalize_for_comparison(self, text: str) -> str:
        ...
        stop_words = {'der', 'die', 'das', 'und', 'oder', 'in', 'zu', 'von', 'mit', 'für'}
    ...
    def _analyze_length_distribution(self, data: Dict[str, Any]) -> Dict[str, Dict[str, float]]:
        ...
        for entity_type in entity_types:
            ...
            for field in text_fields:
                ...
                if lengths:
                    ...
                    length_stats[entity_type][field] = {
                        "mean": 0.0, "median": 0.0, "min": 0, "max": 0, "std": 0.0, "count": 0
                    }
    ...
    def _check_language_consistency(self, data: Dict[str, Any]) -> Dict[str, float]:
        ...
        if all_languages:
            ...
            language_stats["inconsistency_rate"] = 1.0 - (primary_count / total_entities)
    ...
    def _simple_language_check(self, data: Dict[str, Any]) -> Dict[str, float]:
        ...
        german_indicators = {
            'der', 'die', 'das', 'und', 'oder', 'mit', 'für', 'von', 'zu', 'in',
            'auf', 'an', 'bei', 'durch', 'über', 'unter', 'nach', 'vor', 'zwischen',
            'ä', 'ö', 'ü', 'ß', 'straße', 'platz', 'stadt', 'gemeinde'
        }
        
        english_indicators = {
            'the', 'and', 'or', 'with', 'for', 'of', 'to', 'in', 'on', 'at',
            'by', 'through', 'over', 'under', 'after', 'before', 'between',
            'street', 'place', 'city', 'community'
        }
        ...
        for entity_type in entity_types:
            for entity in data.get(entity_type, []):
                ...
                if len(combined_text) > 10:
                    ...
                    german_matches = sum(1 for word in german_indicators if word in combined_text)
                    english_matches = sum(1 for word in english_indicators if word in combined_text)
        ...
        primary_lang = "de" if german_score > english_score else "en" if english_score > 0 else "unknown"
        primary_count = max(german_score, english_score)
        inconsistency = 1.0 - (primary_count / total_texts) if total_texts > 0 else 0.0
        
        return {
            "primary_language": primary_lang,
            "language_distribution": {"de": german_score / total_texts, "en": english_score / total_texts} if total_texts > 0 else {},
            ...
            for entity in data.get(entity_type, []):
                ...
                for field, text in content.items():
                    ...
                    if re.search(r'\s{2,}', text):
                        issues.append(f"{entity_id}.{field}: Multiple consecutive spaces")
                    
                    if text != text.strip():
                        issues.append(f"{entity_id}.{field}: Leading/trailing whitespace")
                    ...
                    if field in ["title", "name"] and text.lower() == text:
                        issues.append(f"{entity_id}.{field}: Title should be capitalized")
                    
                    # Check for very long lines (potential formatting issue)
                    lines = text.split('\n')
                    for i, line in enumerate(lines):
                        if len(line) > 200:
                            issues.append(f"{entity_id}.{field} line {i+1}: Very long line ({len(line)} chars)")
            ...
            for entity in data.get(entity_type, []):
                ...
                if entity_type in ["measures", "indicators"]:
                    ...
                    if not description or not description.strip():
                        outliers["empty_required"].append(f"{entity_id}: Missing description")
                ...
                for field, text in content.items():
                    ...
                    if field in ["title", "name"]:
                        if text_len < 5:
                            outliers["too_short"].append(f"{entity_id}.{field}: {text_len} chars")
                        elif text_len > 200:
                            outliers["too_long"].append(f"{entity_id}.{field}: {text_len} chars")
                    ...
                    if re.search(r'\b(test|TODO|FIXME|placeholder)\b', text, re.IGNORECASE):
                        outliers["suspicious_content"].append(f"{entity_id}.{field}: Contains test/placeholder text")
                    
                    # Check for repeated characters (possible OCR errors)
                    if re.search(r'(.)\1{5,}', text):
                        outliers["suspicious_content"].append(f"{entity_id}.{field}: Repeated characters")
    ...
    def get_content_quality_score(self, stats: ContentStats) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_issues": len(stats.duplicate_text) + len(stats.normalization_issues) + total_outliers
        }