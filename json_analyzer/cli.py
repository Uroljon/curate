...
from .visualizer import TerminalVisualizer, HTMLReportGenerator
...
"""
    ...
    analyze_parser = subparsers.add_parser(
        "analyze",
        help="Analyze a single JSON file"
    )
    ...
    compare_parser.add_argument(
        "before",
        type=str,
        help="Earlier JSON file"
    )
    compare_parser.add_argument(
        "after",
        type=str,
        help="Later JSON file"
    )
    compare_parser.add_argument(
        "--format", "-f",
        choices=["terminal", "html", "json"],
    ...
    batch_parser = subparsers.add_parser(
        "batch",
        help="Analyze multiple JSON files"
    )
    ...
    report_parser = subparsers.add_parser(
        "report",
        help="Generate detailed report from analysis"
    )
    report_parser.add_argument(
        "analysis_file",
        type=str,
        help="Analysis JSON file (from --save-json)"
    )
    report_parser.add_argument(
        "--format", "-f",
...
def cmd_compare(args, config: AnalyzerConfig):
    ...
    if args.format == "terminal":
        ...
        output_path = args.output or f"comparison_{before_path.stem}_vs_{after_path.stem}.html"
        generator = HTMLReportGenerator()
        ...
        output_path = args.output or f"comparison_{before_path.stem}_vs_{after_path.stem}.json"
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(result.model_dump_json(indent=2))
...
def cmd_batch(args, config: AnalyzerConfig):
    ...
    if not args.quiet:
        ...
        avg_score = sum(r.get("overall_score", 0) for r in results) / total_files if results else 0
...
def save_results_csv(results: list, output_path: Path):
    ...
    field_order = ["file_path", "overall_score", "grade", "format", "total_entities", "total_connections", "critical_issues", "analysis_time_ms", "error"]
